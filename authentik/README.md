...
2. Prepare an external PostgreSQL database (see [DOCS.md](DOCS.md)).
3. Install authentik.
4. Enter the database connection details in the configuration.
5. Start the add-on and open the interface on port `9000`, then complete the
   initial setup at `/if/flow/initial-setup/`.

authentik keeps its state in external PostgreSQL. Uploaded files, certificates,
custom templates, and the generated secret key persist in the add-on
